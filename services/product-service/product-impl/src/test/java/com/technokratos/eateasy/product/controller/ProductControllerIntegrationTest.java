...
class ProductControllerIntegrationTest {
    ...
    final ProductRequest BASE_PRODUCT = new ProductRequest(
            "Apple",
            "Fresh red apples",
    ...
    void updateProductQuantityTest() {
        UUID productId = createTestProduct().id();
        Integer addValue = 50;
        Integer oldQuantity = jdbcTemplate.queryForObject(
        ...
        Integer updatedQuantity = jdbcTemplate.queryForObject(
                "SELECT quantity FROM product WHERE id = :id",
                new MapSqlParameterSource("id", productId),
                Integer.class
        );
    ...
    void updateProductTest() {
        UUID productId = createTestProduct().id();
        ProductRequest updateRequest = testProduct.builder()
                .title("Green Apple")
