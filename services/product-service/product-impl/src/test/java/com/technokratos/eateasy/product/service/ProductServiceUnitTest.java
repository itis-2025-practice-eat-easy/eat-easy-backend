...
class ProductServiceUnitTest {
    ...
    void updateNotFound() {
        ...
        assertThrows(ProductNotFoundException.class, () -> productService.update(productId, productUpdateRequest));
    }
    ...
    void updateDataViolation() {
        ...
        when(productRepository.update(eq(productId), anyMap()))
                .thenThrow(DataIntegrityViolationException.class);
        assertThrows(DataIntegrityViolationException.class, () -> productService.update(productId, productUpdateRequest));
    }
    ...
    void updateRuntimeException() {
        ...
        when(productRepository.update(eq(productId), anyMap()))
                .thenThrow(new RuntimeException("unexpected"));
        assertThrows(RuntimeException.class, () -> productService.update(productId, productUpdateRequest));
    }
    ...
    void getByCategoryId_allFiltersAndSort_price() {
        ...
        when(productRepository.getByCategoryId(eq(categoryId), eq("price"), eq(0), eq(10),
                eq(new BigDecimal("2.00")), eq(new BigDecimal("10.00")))).thenReturn(List.of(product));
        when(productMapper.toResponse(product)).thenReturn(productResponse);
        List<ProductResponse> result = productService.getByCategoryId(
                categoryId, "price", 0, 10,
    ...
    void getByCategoryId_allFiltersAndSort_popularity() {
        UUID categoryId = UUID.randomUUID();
        when(productRepository.getByCategoryId(eq(categoryId), eq("popularity"), eq(0), eq(10),
                eq(null), eq(null))).thenReturn(List.of(product));
        when(productMapper.toResponse(product)).thenReturn(productResponse);
        List<ProductResponse> result = productService.getByCategoryId(
                categoryId, "popularity", 0, 10, null, null);
        assertEquals(1, result.size());
    }
    ...
    void getByCategoryId_sortByNew() {
        UUID categoryId = UUID.randomUUID();
        when(productRepository.getByCategoryId(eq(categoryId), eq("new"), eq(0), eq(10),
                eq(null), eq(null))).thenReturn(List.of(product));
        when(productMapper.toResponse(product)).thenReturn(productResponse);
        List<ProductResponse> result = productService.getByCategoryId(
                categoryId, "new", 0, 10, null, null);
        assertEquals(1, result.size());
    }

    @Test
    void updateEmptyRequest_doesNothing() {
        when(productMapper.toEntity(productUpdateRequest)).thenReturn(Product.builder().id(productId).build());
        when(productRepository.update(eq(productId), anyMap())).thenReturn(0);
        assertThrows(ProductNotFoundException.class, () -> productService.update(productId, productUpdateRequest));
    }
